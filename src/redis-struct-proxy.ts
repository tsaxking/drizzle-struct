...
export class RedisStructProxyClient<
	Name extends string,
...
> {
	...
	constructor(
		...
		this.connection = redis.createClient(config.target, config.maxQueueLength || 100);
	}
	...
	//
	new<T extends Blank, Name extends string>(
	...
	delete<T extends Blank, Name extends string>(struct: Struct<T, Name>, id: string) {
		return this.request(`${struct.name}.delete`, { id }, z.void(), () => {});
	}
	//
	update<T extends Blank, Name extends string>(
		struct: Struct<T, Name>,
		id: string,
		data: Partial<Structable<T>>
	) {
		return this.request(`${struct.name}.update`, { id, data }, struct.getZodSchema(), (d) =>
			struct.Generator(d as any)
		);
	}
	archive<T extends Blank, Name extends string>(struct: Struct<T, Name>, id: string) {
		return this.request(`${struct.name}.archive`, { id }, z.void(), () => {});
	}
	restore<T extends Blank, Name extends string>(struct: Struct<T, Name>, id: string) {
		return this.request(`${struct.name}.restore`, { id }, z.void(), () => {});
	}
	fromId<T extends Blank, Name extends string>(struct: Struct<T, Name>, id: string) {
		return this.request(`${struct.name}.from-id`, { id }, struct.getZodSchema(), (d) =>
			struct.Generator(d as any)
		);
	}
	...
	fromVhId<T extends Blank, Name extends string>(struct: Struct<T, Name>, vhId: string) {
		return this.request(
	...
	all<T extends Blank, Name extends string>(struct: Struct<T, Name>, config: MultiConfig) {
		return this.request(`${struct.name}.all`, config, z.array(struct.getZodSchema()), (d) =>
			d.map((item: any) => struct.Generator(item as any))
		);
	}
	...
	archived<T extends Blank, Name extends string>(struct: Struct<T, Name>, config: MultiConfig) {
		return this.request(`${struct.name}.archived`, config, z.array(struct.getZodSchema()), (d) =>
			d.map((item: any) => struct.Generator(item as any))
		);
	}
	getVersions<T extends Blank, Name extends string>(struct: Struct<T, Name>, id: string) {
		return this.request(
	...
	restoreVersion<T extends Blank, Name extends string>(struct: Struct<T, Name>, vhId: string) {
		return this.request(`${struct.name}.restore-version`, { vhId }, z.void(), () => {});
	}
	deleteVersion<T extends Blank, Name extends string>(struct: Struct<T, Name>, vhId: string) {
		return this.request(`${struct.name}.delete-version`, { vhId }, z.void(), () => {});
	}
	...
	setAttributes<T extends Blank, Name extends string>(
		...
		return this.request(`${struct.name}.set-attributes`, { id, attributes }, z.void(), () => {});
	}
...
export class RedisStructProxyServer<Name extends string, RedisName extends string> {
	...
	public readonly server: ConnectionServerService<
		RedisName,
		typeof RedisStructProxyServer.structEvents
	>;
	...
	init(config?: { polling?: number }) {
		return attemptAsync(async () => {
			...
			this.server.subscribe('new', (data) => {
				...
				return s
					.new(data.data as any, data.config)
					.unwrap()
					.then((d) => d.data);
			});
			...
			this.server.subscribe('all', async (data) => {
				...
				if (data.data.limit && !isNaN(data.data.limit)) {
					...
					(config as any).offset =
						data.data.offset && !isNaN(data.data.offset) ? data.data.offset : 0;
