...
export class Server {
		...
		this.app.post('/struct', async (req, res) => {
			try {
				...
				const {
					...
					timestamp,
					payload: { data, struct }
				} = structEvent.parse(req.body);
				const apiKey = req.headers['x-api-key'] as string;

				const result = await this.handleStructEvent(apiKey, {
					event,
					timestamp,
					payload: { data, struct }
				});
					...
					case 'from-property':
						{
							const { property, value } = z
								...
								.parse(args);
							stream(
								s.fromProperty(property as any, value, {
									type: 'stream'
								})
							);
						}
					...
					case 'from-universe':
						{
							...
							// }));
							stream(
								s.fromProperty('universe', universe, {
									type: 'stream'
								})
							);
						}
	 ...
	 * @returns
	 */
	async send<T extends Blank, Name extends string>(
		...
		return resolveAll(
			await Promise.all(
				Array.from(this.connections.values()).map(async (c) =>
					attemptAsync(async () => {
						if (config?.if && !config.if(c)) return;
						if (
							await this.config.checkEvent({
								apiKey: c.apiKey,
								event,
								data,
								timestamp: Date.now(),
...
export class Client {
	 ...
	 * @returns {*}
	 */
	send<T extends Blank, Name extends string>(
		struct: Struct<T, Name>,
		event: keyof StructEvent<T>,
		data: Omit<StructEvent<T>[keyof StructEvent<T>], 'timestamp'>,
		config?: {
			if: (connection?: Connection) => boolean;
		}
	) {
	 ...
	 */
	query<T extends Blank, N extends string, Q extends keyof QueryType>(
		struct: Struct<T, N>,
		type: Q,
		args: QueryType[Q]
	): Promise<Result<QueryResponse<T>[Q], Error>> {
		return attemptAsync(async () => {
	...
	private batch: {
		id: number;
		event: keyof StructEvent;
		payload: { struct: string; data: Omit<StructEvent[keyof StructEvent], 'timestamp'> };
		timestamp: number;
	}[] = [];
	/**
									...
									if (data.startsWith('data: ') && data.endsWith('\n\n')) {
										...
										if (em) {
											switch (parsed.event) {
												...
												case 'delete-version':
													{
														em.emit('delete-version', {
															id: parsed.payload.data.id,
															vhId: parsed.payload.data.vhId,
															timestamp: parsed.timestamp,
															source: 'server'
														});
													}
												...
												case 'set-attributes':
													{
														em.emit('set-attributes', {
															id: parsed.payload.data.id,
															attributes: parsed.payload.data.attributes,
															timestamp: parsed.timestamp,
															source: 'server'
														});
													}
